...
for (( hard=1; hard<=$HARDPOSTINGS; hard++ ))
do
        file_one=$((($hard-1)*11+1))
        file_two=$(($file_one+1))
	...
	sed -i s/@@VALFOUR@@/$file_four/g data_target/$file_six.xml
	...
	sed -i s/@@VALTWO@@/$file_two/g data_target/$file_nine.xml

        cp 10.xml data_target/$file_ten.xml
	sed -i s/@@VALONE@@/$file_one/g data_target/$file_ten.xml

        cp 11.xml data_target/$file_eleven.xml
	sed -i s/@@VALONE@@/$file_one/g data_target/$file_eleven.xml
	sed -i s/@@VALTWO@@/$file_two/g data_target/$file_eleven.xml
	sed -i s/@@VALTHREE@@/$file_three/g data_target/$file_eleven.xml
	sed -i s/@@VALFOUR@@/$file_four/g data_target/$file_eleven.xml
	sed -i s/@@VALFIVE@@/$file_five/g data_target/$file_eleven.xml
	sed -i s/@@VALSIX@@/$file_six/g data_target/$file_eleven.xml
	sed -i s/@@VALSEVEN@@/$file_seven/g data_target/$file_eleven.xml
done
