...
class NotLoggedIn(errors.TinyError, errors.AuthenticationError): pass
...
class RPCGateway(object):
    ...
    def __execute(self, obj, method, args=(), auth=True):
        try:
            ...
            raise errors.TinyException(e.message or e.strerror, title='Application Error')

        except RPCException, err:
            if err.type in ('warning', 'UserError'):
                if err.message in ('ConcurrencyException') and len(args) > 4:
                    errors.concurrency(err.message, err.data, args)
                else:
                    errors.warning(err.data)
            else:
                errors.error('Application Error', err.backtrace)

        except Exception, e:
            errors.error('Application Error', str(e))
...
class RPCSession(object):
    ...
    def __init__(self, host, port, protocol='socket', storage={}):
        ...
        if protocol in ('http', 'https'):
            ...
            raise errors.message("Unsupported protocol.")
    ...
    def context_reload(self):
        ...
        if self.storage.get('client_timezone'):
            ...
            try:
                ...
                raise errors.warning('You select a timezone but OpenERP could not find pytz library!\nThe timezone functionality will be disable.')
    ...
    def execute(self, obj, method, *args):
        if not self.is_logged():
            raise NotLoggedIn('Not logged...', 'Authorization Error')
