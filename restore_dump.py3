...
class RBIndex(dbmatch):
    ...
    def __init__(self, rb_name, include_dbs = ''):
        ...
        if max_date != empty_date:
            ...
            for dump in pattern.findall(str(content)):
                if self.match(dump):
    ...
    def write_dump(self, db, file_desc):
        f = urllib.request.urlopen('%s/%s' % (self.rb_name, db))
        while True:
            data = f.read(10*1024)
            if data == b'':
                break
...
class ApacheIndexes(object):
    ...
    def write_dump(self, db, file_desc):
        request = urllib.request.Request("%s://%s/%s" % (self.proto, self.host, db))
        if self.user and self.password:
            base64string = b64encode(bytes('%s:%s' % (self.user, self.password), 'utf8')).decode('utf-8')
...
class Web(object):
    ...
    def write_dump(self, db, file_desc):
        cnx = httplib2.Http(disable_ssl_certificate_validation=True)
        resp, content = cnx.request(self.backup_url, 'POST', body=urllib.parse.urlencode({'dbname': db, 'password': self.password}), headers=self.headers)
...
def get_harware_id():
    ...
    mac.sort()
    return hashlib.md5((''.join(mac)).encode('utf8')).hexdigest()
...
def do_upgrade(port, database, uf_pass):
    ...
    while True:
        try:
            sock = xmlrpc.client.ServerProxy('http://%s:%s/xmlrpc/common' % ('127.0.0.1', port))
            sock.login(database, 'admin', uf_pass)
            return True
        except xmlrpc.client.Fault as e:
            if 'ServerUpdate' in e.faultString:
...
def connect_and_sync(dbs_name, sync_port, sync_run, sync_db=False, uf_pass='admin'):
    ...
    for db in dbs_name:
        ...
        if sync_db:
            try:
                sock = xmlrpc.client.ServerProxy('http://%s:%s/xmlrpc/common' % ('127.0.0.1', sync_port))
                uid = sock.login(db, 'admin', uf_pass)
                sock = xmlrpc.client.ServerProxy('http://%s:%s/xmlrpc/object' % ('127.0.0.1', sync_port))
                conn_ids = sock.execute(db, uid, uf_pass, 'sync.client.sync_server_connection', 'search', [])
                sock.execute(db, uid, uf_pass, 'sync.client.sync_server_connection', 'write', conn_ids, {'automatic_patching': False, 'password': uf_pass})
                sock.execute(db, uid, uf_pass, 'sync.client.sync_server_connection', 'connect')
                if o.sync_run:
                    sock.execute(db, uid, uf_pass, 'sync.client.entity', 'sync_manual_threaded')
...
update res_users set password='"""+bcrypt.hash(o.uf_password)+"""';
